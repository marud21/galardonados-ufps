...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($contrasena, $contrasenaHash)) {
            ...
            $_SESSION['administrador_id'] = $row['id']; // Guardar el ID del evaluador en la sesión
            $_SESSION['administrador_usuario'] = $row['usuario']; // Guardar el usuario del evaluador en la sesión
            
            // Puedes agregar otras variables de sesión si las necesitas

            echo "Acceso concedido!";
            // Redirigir a la página principal del evaluador
           header("refresh:3;url=../vista/estadoconvocatoria.php"); // Cambia la URL de redirección
        } else {
